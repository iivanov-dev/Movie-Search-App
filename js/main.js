...
if (!inputFilmNameNode || !searchFilmBtnNode || !noFilmNode || !allFilmsNode) {
    ...
    throw new Error('Критическая ошибка: один или несколько DOM элементов не найдены');
}
...
function renderFilms(filmsArray) {
    ...
    filmsArray.forEach(element => {
        ...
        const posterUrl = element.Poster && element.Poster !== 'N/A' ? element.Poster : 'https://via.placeholder.com/300x450?text=No+Image';
...
searchFilmBtnNode.addEventListener('click', function() {
    ...
    fetch(`https://www.omdbapi.com/?s=${encodeURIComponent(inputValue)}&apikey=d69b78ae`)
        .then(response => {
            if (!response.ok) {
